...
public class ContentView extends RelativeLayout {
    ...
    private void fillGridLayout() {
        ...
        for (int i = gridLayout.getChildCount(); i < rows * cols; ++i) {
            ...
            GridLayout.LayoutParams params = new GridLayout.LayoutParams(rowSpec, columnSpec);
            params.setGravity(Gravity.FILL);
            setGridLayoutParams(params, gridLayout.getMeasuredWidth(), gridLayout.getMeasuredHeight());
            gridLayout.addView(itemView, i, params);
        }
    }
    ...
    private void setGridLayoutParams(GridLayout.LayoutParams params, int width, int height) {
        if (width == 0 || height == 0) {
            ...
            if (isShowPageInfoArea && isAlwaysShowPageIndicator) {
                contentViewHeight = contentViewHeight - getContext().getResources().getDimensionPixelSize(R.dimen.contentView_page_indicator_area_padding_top_bottom) -
                        getContext().getResources().getDimensionPixelSize(R.dimen.contentView_page_indicator_area_padding_top_bottom) -
                        getContext().getResources().getDimensionPixelSize(R.dimen.settings_title_text_size);
            }
        ...
        if (wrapContent) {
            ...
            params.bottomMargin = marginSpace;
        }
    }
    ...
    /**
     * must be called at first before setupGridLayout
     */
    public void setItemWrapContent(boolean wrapContent, int marginSpace) {
...
//                v.requestFocus();
    ...
    public void unCheckAllViews() {
        for (int i = 0; i < adapter.getList().size(); i++) {
            GObject temp = adapter.getList().get(i);
            if (temp.hasKey(GAdapterUtil.TAG_SELECTABLE)) {
                temp.putBoolean(GAdapterUtil.TAG_SELECTABLE, false);
            }
            adapter.setObject(i, temp);
        }
